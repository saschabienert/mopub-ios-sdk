...
#import "HZMRAIDParser.h"

#import "HZMRAIDLogger.h"

@interface HZMRAIDParser ()
...
- (NSDictionary *)parseCommandUrl:(NSString *)commandUrl
{
    ...
    [HZMRAIDLogger debug:@"MRAID - Parser" withMessage:[NSString stringWithFormat:@"%@ %@", NSStringFromSelector(_cmd), commandUrl]];
    ...
    if (![self isValidCommand:command]) {
        [HZMRAIDLogger warning:@"MRAID - Parser" withMessage:[NSString stringWithFormat:@"command %@ is unknown", command]];
        return nil;
    }
    
    // Check for valid parameters for the given command.
    if (![self checkParamsForCommand:command params:params]) {
        [HZMRAIDLogger warning:@"MRAID - Parser" withMessage:[NSString stringWithFormat:@"command URL %@ is missing parameters", commandUrl]];
        return nil;
    }

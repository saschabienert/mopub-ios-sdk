...
/**
 *  Returns an ordered set of NSDate*s (in the order specified with the mostRecentFirst param) with the given search parameters and database connection. If the tags set is `nil`, impressions with any tag will count as a match. If adType is `NULL`, impressions with any ad type will count as a match.
 */
- (nonnull NSMutableOrderedSet *) impressionsSince:(nonnull NSDate *)timestamp withCreativeType:(HZCreativeType)creativeType tags:(nullable NSSet *)tags auctionType:(HZAuctionType)auctionType databaseConnection:(nonnull sqlite3 *)db mostRecentFirst:(BOOL)mostRecentFirst;

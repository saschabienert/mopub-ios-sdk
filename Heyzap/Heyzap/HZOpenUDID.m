...
static NSString * const kHZOpenUDIDTSKey = @"OpenUDID_createdTS";
static NSString * const kHZOpenUDIDOOTSKey = @"OpenUDID_optOutTS";
static NSString * const kHZOpenUDIDDomain = @"org.OpenUDID";
static NSString * const kHZOpenUDIDSlotPBPrefix = @"org.OpenUDID.slot.";
static int const kHZOpenUDIDRedundancySlots = 100;

@interface HZOpenUDID (Private)
...
+ (void) _setDict:(id)dict forPasteboard:(id)pboard {
    ...
    [pboard setData:[NSKeyedArchiver archivedDataWithRootObject:dict] forPasteboardType:kHZOpenUDIDDomain];
#else
...
+ (NSMutableDictionary*) _getDictFromPasteboard:(id)pboard {
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR
    id item = [pboard dataForPasteboardType:kHZOpenUDIDDomain];
#else
...
//
+ (NSString*) value {
    return [HZOpenUDID valueWithError:nil];
}

+ (NSString*) valueWithError:(NSError **)error {
    
    if (kHZOpenUDIDSessionCache!=nil) {
        if (error!=nil)
            *error = [NSError errorWithDomain:kHZOpenUDIDDomain
                                         code:kHZOpenUDIDErrorNone
                                     userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"OpenUDID in cache from first call",@"description", nil]];
        return kHZOpenUDIDSessionCache;
    }
    ...
    //
    NSString * appUID = [defaults objectForKey:kHZOpenUDIDAppUIDKey];
    if(appUID == nil)
    {
    ...
    //
    id localDict = [defaults objectForKey:kHZOpenUDIDKey];
    if ([localDict isKindOfClass:[NSDictionary class]]) {
        ...
        openUDID = [localDict objectForKey:kHZOpenUDIDKey];
        myRedundancySlotPBid = [localDict objectForKey:kHZOpenUDIDSlotKey];
        optedOutDate = [localDict objectForKey:kHZOpenUDIDOOTSKey];
    ...
    NSMutableDictionary* frequencyDict = [NSMutableDictionary dictionaryWithCapacity:kHZOpenUDIDRedundancySlots];
    for (int n=0; n<kHZOpenUDIDRedundancySlots; n++) {
        NSString* slotPBid = [NSString stringWithFormat:@"%@%d",kHZOpenUDIDSlotPBPrefix,n];
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR
        ...
        if (slotPB==nil) {
            ...
            NSDictionary* dict = [HZOpenUDID _getDictFromPasteboard:slotPB];
            NSString* oudid = [dict objectForKey:kHZOpenUDIDKey];
            if (oudid==nil) {
            ...
            NSString* gid = [dict objectForKey:kHZOpenUDIDAppUIDKey];
            if (gid!=nil && [gid isEqualToString:appUID]) {
                ...
                if (optedOut) {
                    optedOutDate = [dict objectForKey:kHZOpenUDIDOOTSKey];
                    optedOut = optedOutDate!=nil;
                }
            }
        }
    ...
    if (openUDID==nil) {
        if (mostReliableOpenUDID==nil) {
            ...
            //
            openUDID = [HZOpenUDID _generateFreshOpenUDID];
        } else {
        ...
        if (localDict==nil) {
            ...
            [localDict setObject:openUDID forKey:kHZOpenUDIDKey];
            [localDict setObject:appUID forKey:kHZOpenUDIDAppUIDKey];
            [localDict setObject:[NSDate date] forKey:kHZOpenUDIDTSKey];
            if (optedOut) [localDict setObject:optedOutDate forKey:kHZOpenUDIDTSKey];
    ...
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        //
        if (localDict) {
            [localDict setObject:availableSlotPBid forKey:kHZOpenUDIDSlotKey];
            saveLocalDictToDefaults = YES;
        }
        ...
        if (openUDID && localDict)
            [HZOpenUDID _setDict:localDict forPasteboard:slotPB];
    }
    
    // Save the dictionary locally if applicable
    //
    if (localDict && saveLocalDictToDefaults)
        [defaults setObject:localDict forKey:kHZOpenUDIDKey];
    ...
    //
    if (optedOut) {
        if (error!=nil) *error = [NSError errorWithDomain:kHZOpenUDIDDomain
                                                     code:kHZOpenUDIDErrorOptedOut
        ...
        kHZOpenUDIDSessionCache = [[NSString stringWithFormat:@"%040x",0] retain];
        return kHZOpenUDIDSessionCache;
    }
    ...
    if (error!=nil) {
        if (isCompromised)
            *error = [NSError errorWithDomain:kHZOpenUDIDDomain
                                         code:kHZOpenUDIDErrorCompromised
    ...
    kHZOpenUDIDSessionCache = [openUDID retain];
    return kHZOpenUDIDSessionCache;
}
...
+ (void) setOptOut:(BOOL)optOutValue {
    ...
    id dict = [defaults objectForKey:kHZOpenUDIDKey];
    ...
    if (optOutValue)
        [dict setObject:[NSDate date] forKey:kHZOpenUDIDOOTSKey];
    else
        [dict removeObjectForKey:kHZOpenUDIDOOTSKey];
    
  	// store the dictionary locally
    [defaults setObject:dict forKey:kHZOpenUDIDKey];
    
    // reset memory cache 
    kHZOpenUDIDSessionCache = nil;
